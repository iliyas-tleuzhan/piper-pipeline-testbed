...
def read_joint_state(timeout_s: float):
    try:
        import rospy
    ...
    names = list(msg.name)
    if len(positions) < 6:
        raise RuntimeError("/joint_states_single did not contain six arm joints")
    return names, positions[:6], msg.header.stamp.to_sec()
...
def main() -> None:
    ...
    names, joints, stamp = read_joint_state(args.timeout)
