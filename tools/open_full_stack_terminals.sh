...
add_remote() {
  ...
  launch_terminal "11 - Remote VLAC Logs" "$PIPELINE_DIR" "ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"vlac|abot-vlac\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No VLAC container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8014 || true; docker logs -f \$name' || ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_FALLBACK_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"vlac|abot-vlac\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No VLAC container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8014 || true; docker logs -f \$name'"

  launch_terminal "12 - Remote YOLO Logs" "$PIPELINE_DIR" "ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"yolo|5090\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No YOLO container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8013 || true; docker logs -f \$name' || ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_FALLBACK_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"yolo|5090\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No YOLO container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8013 || true; docker logs -f \$name'"

  launch_terminal "13 - Remote Spatial Memory Logs" "$PIPELINE_DIR" "ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"spatial|memory\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No Spatial Memory container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8012 || true; docker logs -f \$name' || ssh -o BatchMode=yes -o ConnectTimeout=5 '$REMOTE_FALLBACK_HOST' 'name=\$(docker ps -a --format \"{{.Names}}\" | grep -Ei \"spatial|memory\" | head -n 1); if [ -z \"\$name\" ]; then echo \"No Spatial Memory container found\"; exit 1; fi; docker ps -a --filter name=\$name; ss -ltn | grep :8012 || true; docker logs -f \$name'"
}
