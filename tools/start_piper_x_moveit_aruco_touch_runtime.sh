...
tmux new-session -d -s "$SESSION" -n roscore "docker exec $CONTAINER bash -lc 'source /opt/ros/noetic/setup.bash; export ROS_MASTER_URI=http://localhost:11311 ROS_HOSTNAME=localhost; if timeout 3 rostopic list >/dev/null 2>&1; then echo Reusing existing ROS master; sleep infinity; else roscore; fi'"
sleep 2
...
tmux new-window -t "$SESSION" -n piper_x_feedback "docker exec $CONTAINER bash -lc '$ROS_PREFIX export PYTHONPATH=/root/piper-pipeline-testbed/piper-on-bunker/src:\${PYTHONPATH:-}; python3 /root/piper-pipeline-testbed/piper-on-bunker/scripts/piper_x_passive_socketcan_joint_state_bridge.py --can can0 --joint-topic /piper_x/joint_states'"
tmux new-window -t "$SESSION" -n joint_relay "docker exec $CONTAINER bash -lc '$ROS_PREFIX while ! timeout 2 rostopic echo -n 1 /piper_x/joint_states >/dev/null 2>&1; do echo waiting for /piper_x/joint_states; sleep 1; done; python3 /root/piper-pipeline-testbed/piper-on-bunker/scripts/relay_piper_x_arm_joint_states.py --input-topic /piper_x/joint_states --output-topic /joint_states'"
tmux new-window -t "$SESSION" -n trajectory_controller "docker exec $CONTAINER bash -lc '$ROS_PREFIX export PYTHONPATH=/root/piper-pipeline-testbed/piper-on-bunker/src:\${PYTHONPATH:-}; while ! timeout 2 rostopic echo -n 1 /joint_states >/dev/null 2>&1; do echo waiting for /joint_states; sleep 1; done; python3 /root/piper-pipeline-testbed/piper-on-bunker/scripts/piper_x_moveit_sdk_trajectory_controller.py --feedback-topic /joint_states --can can0 --speed-percent $PIPER_X_MOVEIT_SPEED_PERCENT --command-rate-hz $PIPER_X_MOVEIT_COMMAND_RATE_HZ'"
tmux new-window -t "$SESSION" -n move_group "docker exec $CONTAINER bash -lc '$ROS_PREFIX while ! timeout 2 rostopic echo -n 1 /joint_states >/dev/null 2>&1; do echo waiting for /joint_states; sleep 1; done; roslaunch piper_x_moveit_config planning_only.launch use_rviz:=false info:=true'"
tmux new-window -t "$SESSION" -n d435i_wrist "docker exec $CONTAINER bash -lc '$ROS_PREFIX cd /root/ABot-Claw/robot_layer/arm_piper/agent_server; python3 realsense_d555_py_publisher.py --camera wrist_camera --serial $CAMERA_SERIAL --width 640 --height 480 --fps 15'"
tmux new-window -t "$SESSION" -n image_rectify "docker exec $CONTAINER bash -lc '$ROS_PREFIX while ! timeout 2 rostopic echo -n 1 /wrist_camera/color/image_raw >/dev/null 2>&1; do echo waiting for wrist raw image; sleep 1; done; rosrun image_proc image_proc __name:=image_proc __ns:=/wrist_camera/color'"
tmux new-window -t "$SESSION" -n aruco "docker exec $CONTAINER bash -lc '$ROS_PREFIX export PYTHONPATH=/root/piper-pipeline-testbed/piper-on-bunker/src:\${PYTHONPATH:-}; while ! timeout 2 rostopic echo -n 1 /wrist_camera/color/image_rect_color >/dev/null 2>&1; do echo waiting for rectified wrist image; sleep 1; done; python3 /root/piper-pipeline-testbed/piper-on-bunker/scripts/piper_x_aruco_pose_node.py _image_topic:=/wrist_camera/color/image_rect_color _camera_info_topic:=/wrist_camera/color/camera_info _image_geometry_mode:=rectified _pose_topic:=/aruco_simple/pose _debug_image_topic:=/aruco_simple/debug_image _dictionary:=$MARKER_DICTIONARY _marker_id:=$MARKER_ID _marker_size_m:=$MARKER_SIZE_M _camera_frame:=wrist_camera_color_optical_frame _marker_frame:=aruco_marker_frame'"
